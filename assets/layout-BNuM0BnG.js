import{r as e}from"./auth-Clpj9KIu.js";import{n as t,t as n}from"./icons-BIAwcOnk.js";function r(e){return`${window.location.hostname.includes(`github.io`)?`/biblion`:``}${e.startsWith(`/`)?e:`/${e}`}`}function i(i,a=`dashboard`,o){let s=document.getElementById(`app`);if(!s)return;let c=o?.role===`proprietario`,l=[{id:`dashboard`,label:`Dashboard`,icon:`dashboard`,href:r(c?`/pages/dashboard.html`:`/cliente/dashboard-cliente.html`)}];c&&l.push({id:`autores`,label:`Autores`,icon:`author`,href:r(`/pages/autores/index.html`)},{id:`livros`,label:`Livros`,icon:`book`,href:r(`/pages/livros/index.html`)}),s.innerHTML=`
    <div class="flex min-h-screen">
      <aside class="w-64 bg-surface-light dark:bg-surface-dark border-r border-border-light dark:border-border-dark hidden md:flex flex-col fixed inset-y-0">
        ...
        <nav class="flex-1 px-4 py-4 space-y-1">
          ${l.map(e=>`
            <a href="${e.href}" class="flex items-center gap-3 px-4 py-2 rounded-lg transition-colors ${a===e.id?`bg-primary text-white`:`text-text-secondary-light dark:text-text-secondary-dark hover:bg-slate-100 dark:hover:bg-slate-800`}">
              ${n(e.icon)}
              <span>${e.label}</span>
            </a>
          `).join(``)}
      ...
      <main class="flex-1 md:ml-64 bg-background-light dark:bg-background-dark">
        <header class="h-16 bg-surface-light dark:bg-surface-dark border-b border-border-light dark:border-border-dark flex items-center justify-between px-6 sticky top-0 z-10">
          ...
          <div class="hidden md:block">
            <h2 class="text-lg font-semibold capitalize">${a}</h2>
          </div>
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="text-right hidden sm:block">
              <p class="text-sm font-medium">${o?.nome||`Usuário`}</p>
              <p class="text-xs text-text-secondary-light dark:text-text-secondary-dark">${o?.role===`proprietario`?`Administrador`:`Leitor`}</p>
            </div>
  ...
  `,t();let u=document.getElementById(`themeToggle`);u&&u.addEventListener(`click`,()=>{document.documentElement.classList.toggle(`dark`),localStorage.setItem(`theme`,document.documentElement.classList.contains(`dark`)?`dark`:`light`)}),localStorage.getItem(`theme`)===`dark`||!localStorage.getItem(`theme`)&&window.matchMedia(`(prefers-color-scheme: dark)`).matches?document.documentElement.classList.add(`dark`):document.documentElement.classList.remove(`dark`),document.getElementById(`logoutBtn`)?.addEventListener(`click`,()=>{e()})}export{i as t};