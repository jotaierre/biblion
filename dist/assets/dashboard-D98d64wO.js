import{c as e,t}from"./auth-XPfV9UYX.js";import"./icons-1bdniJDT.js";import{t as n}from"./layout-HDTcLn4t.js";async function r(){let{session:r,profile:i}=await t();if(!r||i?.role!==`proprietario`){i?.role===`cliente`&&(window.location.href=`/cliente/dashboard-cliente.html`);return}let{count:a}=await e.from(`autores`).select(`*`,{count:`exact`,head:!0}),{count:o}=await e.from(`livros`).select(`*`,{count:`exact`,head:!0});n(`
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      <div class="card flex items-center gap-4">
        ...
        <div>
          ...
          <p class="text-2xl font-bold">${a||0}</p>
        </div>
      </div>
