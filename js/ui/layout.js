      ...
      <aside class="w-64 bg-surface-light dark:bg-surface-dark border-r border-border-light dark:border-border-dark hidden md:flex flex-col fixed inset-y-0">
        ...
        <nav class="flex-1 px-4 py-4 space-y-1">
          ${sidebarItems.map(item => `
            <a href="${item.href}" class="flex items-center gap-3 px-4 py-2 rounded-lg transition-colors ${activePage === item.id ? 'bg-primary text-white' : 'text-text-secondary-light dark:text-text-secondary-dark hover:bg-slate-100 dark:hover:bg-slate-800'}">
              ${heroicon(item.icon)}
      ...
      <main class="flex-1 md:ml-64 bg-background-light dark:bg-background-dark">
        ...
        <header class="h-16 bg-surface-light dark:bg-surface-dark border-b border-border-light dark:border-border-dark flex items-center justify-between px-6 sticky top-0 z-10">
          ...
          <div class="hidden md:block">
            <h2 class="text-lg font-semibold capitalize">${activePage}</h2>
          </div>
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="text-right hidden sm:block">
              <p class="text-sm font-medium">${profile?.nome || 'Usuário'}</p>
              <p class="text-xs text-text-secondary-light dark:text-text-secondary-dark">${profile?.role === 'proprietario' ? 'Administrador' : 'Leitor'}</p>
            </div>
